...
import { Plane, Hotel, Car, Train, MapPin, Menu, X, User, LogIn, Umbrella, House } from 'lucide-react';
...
const Navbar = ({setIsPopupOpen}) => {
    ...
    {[
        ...
        { path: '/holiday', icon: <Umbrella size={16} />, label: 'Holiday Packages' },
    ...
    {[
        { path: '/', icon: <House size={16} />, label: 'Home' },
        { path: '/flight-booking', icon: <Plane size={16} />, label: 'Flights' },
        ...
        { path: '/holiday', icon: <Umbrella size={16} />, label: 'Holiday Packages' },
        { path: '/destination', icon: <MapPin size={16} />, label: 'Destinations' },
    ].map((item) => (
        <Link key={`${item.path}-${item.label}`} to={item.path} onClick={() => setMobileMenuOpen(false)}>
            <li className={styles.mobileNavLink}>
                ...
                <div className={styles.mobileAuthButtons}>
                    <button className={styles.loginButton} onClick={() => setMobileMenuOpen(false)}>
                        <LogIn size={16} />
                        <span>Login</span>
                    </button>
                    <button className={styles.signupButton} onClick={() => setMobileMenuOpen(false)}>
                        <User size={16} />
                        <span>Sign Up</span>
                    </button>
                </div>
